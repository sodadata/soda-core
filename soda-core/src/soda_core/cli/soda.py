...
class CLI:
    ...
    def _publish_contract(self, contract_file_paths: Optional[list[str]]):
        print(f"Publishing contracts {contract_file_paths}")
    ...
    def _test_data_source(self, data_source_file_path: str):
        ...
        if error_message:
            print(
                f"Could not connect {Emoticons.POLICE_CAR_LIGHT} using data source '{data_source_file_path}': "
                f"{error_message}"
            )
            exit(2)
        else:
