...
# it executes the module-level ``register_family(...)`` call that wires
# the ``contract`` kind into the check-collection family registry. Any
# caller that imports anything from ``soda_core.contracts`` (the public
# facade) gets a fully-registered ``contract`` family. The previous lazy
