...
class SodaCloud:
    ...
    def publish_contract(self, contract_yaml: Optional[ContractYaml]) -> ContractPublicationResult:
        ...
        logger.info(
            f"Publishing {Emoticons.SCROLL} contract {contract_yaml.check_collection_yaml_source.file_path} "
            f"{Emoticons.FINGERS_CROSSED}"
        )
        contract_yaml_str_original = contract_yaml.check_collection_yaml_source.yaml_str_original
        contract_local_file_path = contract_yaml.check_collection_yaml_source.file_path
