...
@pytest.mark.parametrize(
    "check",
    [
        pytest.param("- missing_count(cat) = 0", id="missing_count"),
        pytest.param("- missing_percent(cat) = 0", id="missing_percent"),
        pytest.param(
            """- invalid_count(cat) = 0:
                   valid format: email""",
            id="invalid_count",
        ...
        pytest.param("- duplicate_count(cat) = 0", id="duplicate_count"),
        pytest.param("- duplicate_percent(cat) = 0", id="duplicate_percent"),
        pytest.param("- values in (cst_size) must exist in {{another_table_name}} (cst_size)", id="reference"),
        pytest.param(
            """- failed rows:
...
def test_dataset_checks(check: str, data_source_fixture: DataSourceFixture):
...
def test_for_each_checks(check: str, data_source_fixture: DataSourceFixture):
