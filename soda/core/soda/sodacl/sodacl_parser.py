...
class SodaCLParser(Parser):
    ...
    def __parse_tables(self, header_content, tables_cfg):
        tables_cfg.data_source_name = header_content.get("data_source")
        tables = header_content.get("tables")
        if isinstance(tables, list):
            for table in tables:
                if table.startswith("exclude "):
                    exclude_table_expression = table[len("exclude ") :]
                    tables_cfg.exclude_tables.append(exclude_table_expression)
                else:
                    if table.startswith("include "):
                        include_table_expression = table[len("include ") :]
                    else:
                        include_table_expression = table
                    tables_cfg.include_tables.append(include_table_expression)
        else:
            self.logs.error(
    ...
    def __parse_discover_tables_section(self, header_str, header_content):
        tables_cfg = DatasourceTablesCfg(self.data_source_name, self.location)
        self.__parse_tables(header_content, tables_cfg)
        self.get_data_source_scan_cfgs().add_discover_tables_cfg(tables_cfg)
    ...
    def __parse_sample_datasets_section(self, header_str, header_content):
        tables_cfg = DatasourceTablesCfg(self.data_source_name, self.location)
        self.__parse_tables(header_content, tables_cfg)
        self.get_data_source_scan_cfgs().add_sample_tables_cfg(tables_cfg)
