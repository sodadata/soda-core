...
def test_prophet_model_skip_measurements(
    ...
    pd.testing.assert_frame_equal(
        filtered_time_series_data, expected_filtered_time_series_data, check_dtype=False
    )
...
def test_feedback_processor(historic_check_results, expected_processed_feedback):
    ...
    feedback_processor = FeedbackProcessor(
        params=PROPHET_MODEL_PARAMS, df_historic=df_historic, logs=LOGS
    )
...
def test_feedback_processor_prophet_model_skip_measurements(
    historic_check_results, expected_filtered_time_series
):
    df_historic = pd.DataFrame(historic_check_results)
    df_historic["ds"] = pd.to_datetime(df_historic["ds"])
