...
class SqlServerDataSourceTestHelper(DataSourceTestHelper):
    ...
    def drop_test_schema_if_exists_sql(self):
        ...
        BEGIN
            SELECT @SQL = 'DROP PROCEDURE [{schema_name}].[' + RTRIM(@name) +']'
            EXEC (@SQL)
        ...
        WHILE @name IS NOT NULL
        BEGIN
            SELECT @SQL = 'DROP VIEW [{schema_name}].[' + RTRIM(@name) +']'
            EXEC (@SQL)
        ...
        WHILE @name IS NOT NULL
        BEGIN
            SELECT @SQL = 'DROP FUNCTION [{schema_name}].[' + RTRIM(@name) +']'
            EXEC (@SQL)
        ...
        BEGIN
            ...
            BEGIN
                SELECT @SQL = 'ALTER TABLE [{schema_name}].[' + RTRIM(@name) +'] DROP CONSTRAINT [' + RTRIM(@constraint) +']'
                EXEC (@SQL)
