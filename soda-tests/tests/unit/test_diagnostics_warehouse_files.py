...
def test_normalize_string_wraps_as_primary_only():
    ...
    assert result == DiagnosticsWarehouseFiles(primary_path="/path/to/dwh.yaml", metadata_dwh_file_path=None)
...
def test_normalize_empty_string_wraps_as_primary_only():
    ...
    assert result == DiagnosticsWarehouseFiles(primary_path="", metadata_dwh_file_path=None)


def test_normalize_instance_passes_through():
    instance = DiagnosticsWarehouseFiles(primary_path="/p.yaml", metadata_dwh_file_path="/m.yaml")
...
def test_is_empty():
    ...
    assert DiagnosticsWarehouseFiles(metadata_dwh_file_path="/m").is_empty is False
